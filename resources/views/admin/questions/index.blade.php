...
<?php
    $count = 0;
    $QuestionsCreate = check_permission('Questions','create');
    $QuestionsEdit = check_permission('Questions','edit');
    $QuestionsDelete = check_permission('Questions','destroy');
?>
                        ...
                        <table id="leads" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th>Course </th>
                                    <th>Questions</th>
                                    @if($QuestionsEdit == 1 || $QuestionsDelete == 1)
                                    <th>Action</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    <td>{{$count}}</td>
                                    <td>{{ $Questionsdata->course->name }} </td>
                                    <td>{{ $Questionsdata['question'] }} </td>
                                   
                                    <td>
                                        @if($QuestionsEdit == 1)
                                        <a class="btn bg-primary" href="{{route('questions.edit',$Questionsdata['id'])}}"><i
                                                class="fas fa-edit"></i></a>
                                        @endif
                                        @if($QuestionsDelete == 1)
                                        <button class="btn btn-danger deleteButton" type="button"
                                            data-id="{{ $Questionsdata->id }}"><i class="fas fa-trash"></i></button>
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
